...
class WebSocket
{
    ...
    public function __construct()
    {
        \Co\Run(function () {
            ...
            $this->server = new \Co\Http\Server($this->host, $this->port, false);

            $this->server->handle('/', function ($request, $response) {
                $this->redis->lpush('user_chat_record', "你好呀");
                $this->redis->incr('online_num');
                $data = [
                    'request' => $request,
                    'response' => $response,
                    'online_num' => $this->redis->get('online_num'),
                    'chat_list' => $this->redis->lrange('user_chat_record', 0, -1),
                ];
                $response->end(json_encode($data));
            });

            $this->server->handle('/websocket', function ($request, $ws) {
                $ws->upgrade();
                while (true) {
                    $frame = $ws->recv();
                    if ($frame === false) {
            ...
            $this->server->handle('/favicon.ico', function ($req, $resp) {
                $resp->status(404);
            });

            $this->server->handle('/name', function ($request, $response) {
                $this->getWebSocketName();
            });

            $this->server->handle('/stop', function ($request, $response) {
    ...
    {
        //查询聊天记录
        $this->redis->lrange('user_chat_record', 0, -1);
        //在线人数加1
        $this->redis->incr('online-num');
        $res = [
            'type' => 'init',
            'sendMessage' => '',
            'replyMessage' => '连接服务器成功！',
            'data' => $request,
            'online_num' => $this->redis->get('online-num'),
    ...
    public function onMessage($websocket, $frame)
    {
        ...
        //保存聊天记录
        $this->redis->lpush('user_chat_record', $data['content']);
        if ($data['type'] == 'init') {
            #TODO
        } else {
            $res['sendMessage'] = $data['content'];
            if ($data['content'] == "你好") {
                $res['replyMessage'] = '欢迎光临我的小站！';
            } elseif (strtoupper($data['content']) == "PHP") {
                $res['replyMessage'] = '<a href="http://www.imliaogm.com/index/detail/2" target="_blank" style="cursor: pointer;color: blue;">点击查看PHP应该学习的技术</a>';
            } else {
                $res['replyMessage'] = "服务器返回的信息为：" . $data['content'];
            }
            $res['type'] = 'reply';
        }
    ...
    {
        //在线人数减1
        $this->redis->decr('online-num');
        $res = [
            'type' => 'init',
            'sendMessage' => '',
            'replyMessage' => '客户端{$fd}退出聊天室！',
            'online_num' => 12,
            'time' => date('Y-m-d H:i:s', time()),
        ];
        $websocket->push($fd, json_encode($res));
    }
}
